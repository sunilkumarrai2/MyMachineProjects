 ...
 */
angular.module("Portfolio_Frontend").controller("creativeCtrl", function($scope,Lightbox,imageService,$timeout) {
    ...
    $scope.sorting =  function(index){
        ...
        imageService.updateOrder("creative_images",newdata).then(function(response){
    ...
    $scope.$on('emitedFromBaseToCreative', function(event, args) {
        if(args === "creative"){
            imageService.getCreativeImages().then(function(response) {
                $scope.images = response;
                });
