 ...
 */
angular.module("Portfolio_Frontend").controller("specialFxCtrl", function($scope,Lightbox,imageService,$timeout) {
    ...
    $scope.sorting =  function(index){
        ...
        imageService.updateOrder("specialfx_images",newdata).then(function(response){
    ...
    $scope.$on('emitedFromBaseToSpecialfx', function(event, args) {
        if(args === "specilfx"){
            imageService.getSpecialfxImages().then(function(response) {
                $scope.images = response;
                });
